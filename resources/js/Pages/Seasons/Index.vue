    ...
    <AppLayout title="Seasons">
        <div
            ...
            <div class="relative z-10 px-6 py-10 text-center">
                <h1
                    class="mb-2 text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-200">
                    Seasons</h1>
        ...
        <div class="max-w-4xl px-4 mx-auto sm:px-6 lg:px-8">
            <div class="space-y-8">
                <div v-for="season in seasons" :key="season.id"
                    class="relative flex flex-col md:flex-row items-center md:items-stretch p-6 md:p-8 bg-white/90 dark:bg-gray-900/90 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 hover:scale-[1.01] transition">
                    <div class="flex flex-col justify-between flex-1 w-full gap-6 md:flex-row md:items-center">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap gap-6 mb-2 text-sm text-gray-700 dark:text-gray-300">
                                <div>
                                    <span class="font-semibold">Start:</span> {{ new
                                        Date(season.starts_at).toLocaleDateString() }}
                                </div>
                                <div>
                        ...
                        <div class="flex flex-col justify-center items-end gap-2 min-w-[160px]">
                            <Link :href="route('seasons.show', season.id)"
                                class="inline-block px-6 py-3 font-semibold text-white transition shadow bg-gradient-to-r from-indigo-500 to-pink-500 rounded-xl hover:from-indigo-600 hover:to-pink-600">
                            View Season
                            </Link>
                            <div v-if="season.status === 'completed'"
                                class="flex items-center gap-3 px-4 py-2 mt-4 font-semibold text-yellow-900 bg-yellow-100 shadow rounded-xl dark:bg-yellow-900/30 dark:text-yellow-200">
                                <template v-if="season.winner">
                                    <img v-if="season.winner.logo_url" :src="season.winner.logo_url"
                                        class="w-8 h-8 border-2 border-yellow-400 rounded-full"
                                        :alt="season.winner.name">
                                    <svg v-else class="w-8 h-8 text-yellow-400" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M8 21h8M12 17v4M17 5V3a1 1 0 00-1-1H8a1 1 0 00-1 1v2M21 5h-4M7 5H3m0 0a9 9 0 0018 0" />
                                    </svg>
