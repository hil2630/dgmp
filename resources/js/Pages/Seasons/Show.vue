    ...
    <AppLayout :title="season.name">
        <div class="relative bg-gradient-to-br from-blue-900/50 to-gray-900/50 min-h-[180px] flex flex-col justify-center items-center shadow-xl mb-12 overflow-hidden">
            ...
            <div class="relative z-10 px-6 py-10 text-center">
                <h2 class="flex items-center justify-center gap-3 mb-2 text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-200 md:text-4xl">
        ...
        <div class="grid grid-cols-1 gap-8 px-4 mx-auto max-w-7xl sm:px-6 lg:px-8 md:grid-cols-3">
            <!-- Season Info -->
            <div class="relative flex flex-col p-8 overflow-hidden shadow-2xl md:col-span-1 bg-white/80 dark:bg-gray-900/80 rounded-2xl">
                <div class="absolute w-24 h-24 rounded-full -top-8 -right-8 bg-gradient-to-br from-indigo-400 to-pink-400 opacity-20 blur-2xl"></div>
                <h3 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Season Details</h3>
                <dl class="space-y-4">
                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Start Date</dt>
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ new Date(season.starts_at).toLocaleDateString() }}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ season.teams.length }} teams participating</dd>
                    </div>
                </dl>
                <div class="mt-8">
                    <h4 class="mb-2 font-semibold text-gray-700 text-md dark:text-gray-300">Time Remaining</h4>
                    <p v-if="season.status === 'active'" class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">
                        {{ Math.floor(season.time_remaining / 3600) }}h {{ Math.floor((season.time_remaining % 3600) / 60) }}m
                    </p>
                    <p v-else class="text-sm text-gray-500 dark:text-gray-400">
                        {{ season.status === 'pending' ? 'Not started yet' : 'Season completed' }}
                    </p>
                </div>
                <div v-if="successMessage" class="px-4 py-2 mb-4 font-semibold text-center text-green-800 bg-green-100 rounded-lg">
                    {{ successMessage }}
                </div>
                <div v-if="team && isTeamOwner() && season.status === 'pending'" class="mt-8">
                    <button v-if="!isTeamSignedUp()" @click="signUpTeam" :disabled="processing" class="w-full px-6 py-3 text-lg font-semibold text-white transition bg-indigo-600 shadow rounded-xl hover:bg-indigo-700 disabled:opacity-50">
                        <span v-if="processing">Signing up...</span>
                        <span v-else>Sign Up Team</span>
                    </button>
                    <button v-else @click="removeSignupTeam" :disabled="processing" class="w-full px-6 py-3 text-lg font-semibold text-white transition bg-red-600 shadow rounded-xl hover:bg-red-700 disabled:opacity-50">
            ...
            <!-- Leaderboard -->
            <div class="relative flex flex-col p-8 overflow-hidden shadow-2xl md:col-span-2 bg-white/80 dark:bg-gray-900/80 rounded-2xl">
                <div class="absolute w-24 h-24 rounded-full -bottom-8 -left-8 bg-gradient-to-br from-pink-400 to-indigo-400 opacity-20 blur-2xl"></div>
                ...
                <div class="overflow-x-auto bg-white shadow-lg rounded-xl dark:bg-gray-900">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="sticky top-0 z-10 bg-gradient-to-r from-indigo-200 to-pink-200 dark:from-gray-800 dark:to-gray-700">
                            <tr>
                                <th class="px-6 py-3 text-xs font-bold tracking-wider text-left text-gray-700 uppercase dark:text-gray-300">Rank</th>
                                <th class="px-6 py-3 text-xs font-bold tracking-wider text-left text-gray-700 uppercase dark:text-gray-300">Team</th>
                                <th class="px-6 py-3 text-xs font-bold tracking-wider text-left text-gray-700 uppercase dark:text-gray-300">Runs</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(team, index) in leaderboard" :key="team.id"
                                ...
                                <td class="px-6 py-4 align-middle whitespace-nowrap">
                                    <div class="text-lg font-bold text-indigo-700 dark:text-indigo-300">#{{ index + 1 }}</div>
                                </td>
                                <td class="flex items-center gap-3 px-6 py-4 align-middle whitespace-nowrap">
                                    <template v-if="team.logo_url">
                                        <img :src="team.logo_url" class="w-10 h-10 bg-white border-2 border-indigo-300 rounded-full dark:bg-gray-800" :alt="team.name">
                                    </template>
                                    <template v-else>
                                        <div class="flex items-center justify-center w-10 h-10 text-lg font-bold text-indigo-400 bg-indigo-100 rounded-full dark:bg-gray-700 dark:text-indigo-300">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                            </svg>
                                        </div>
                                    </template>
                                    <span class="font-semibold text-gray-900 text-md dark:text-white">{{ team.name }}</span>
                                </td>
                                <td class="px-6 py-4 align-middle whitespace-nowrap">
                                    <div class="font-semibold text-gray-900 text-md dark:text-white">{{ team.runs_count }}</div>
                                </td>
                            </tr>
